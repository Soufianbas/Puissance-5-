...
int AI::alignement(Board board, string couleur)
{
    ...
    if (maxLineLength >= 4)
    {
        score = 1000; // Score trs lev pour un alignement gagnant
    }
    ...
    {
        score = 1; // Score faible pour une position non stratgique
    }
...
int AI::length(Board board, int ligne, int colonne, string couleur)
{
    ...
    for (int i = 0; i < 5; ++i)
    {
        ...
        { // Parcours jusqu' 3 jetons maximum dans chaque direction
            ...
            // Vrifier si la nouvelle position est valide et si elle contient un jeton du joueur
            ...
            {
                break; // Arrter de parcourir la direction si une case vide ou un jeton adverse est rencontr
            }
        }

        // Compter les jetons dans la direction oppose (-dx, -dy)
        for (int k = 1; k <= 3; ++k)
        {
            int newRow = ligne - k * dx;
            int newCol = colonne - k * dy;
        ...
        // Mettre  jour la longueur maximale trouve dans toutes les directions
        if (length > maxLength)
        {
            maxLength = length;
        }
...
int AI::bestMove(Board board, int depth)
{
    ...
    for (int col = 0; col < board.getNbColonnes(); ++col)
    {
        if (board.isValidMove(col))
        {
            ...
            int score = minimax(newBoard, depth, false); // Appel rcursif  MiniMax
            if (score > bestScore)
            {
                bestScore = score;
                bestMove = col;
